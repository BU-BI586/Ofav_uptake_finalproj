...
- The BD_DE_GO.csv file has two columns of gene id and teh continuous measure of significance. To perform the stanadard GO enrichment analysiss based on Fisher's exact test, we use binary measure (0 or 1, i.e., either significant or not).
...
### MF 
gomwuStats settings were specified with the largest so a GO category will not be considered it contains more than 0.1 of the total number of genes, the smallest is specified so that a GO catefory should contain a minimum of 5 genes to be considered, and the clusterCutHeight is the threshold for merging gene-sharing terms.
```{r,eval=FALSE}
...
- NEW TRANSCRIPTOME, NEW rarified:  0 go terms at 10 % FDR
...
```{r, warning=FALSE, eval=FALSE}
resultsMF=gomwuPlot(input,goAnnotations,goDivision_1,
                  absValue=1,
                  level1=0.1,
                  level2=0.05,
                  level3=0.01,
...
Red is upregulated in coral hosting strain D and blue is downregulated in coral hosting strain D (or upregulated in coral hosting strain B).
...
gomwuStats(input, goDatabase, goAnnotations, goDivision_2,
           ...
           clusterCutHeight=0.25)
```
- NEW TRANSCRIPTOME, new rarefied: 2 GO terms at 10% FDR
...
- NEW TRANSCRIPTOME, new rarefied: 11 GO terms at 10% FDR
...
- Looking at "mitochondrial membrane part" [CC GO term: GO:0044455], and "mitochondrial part" [CC GO term: GO:0044429]
...
- Next, make a variable that contains all the genes that correspond to the GO terms of interest. In this case we will be looking at the genes that correspond to the parent GO terms of Mitochondrial Membrane Part (GO:0044455) and Mitochondrial Part (GO:0044429) in CC
```{r, warning=FALSE, eval=FALSE}
...
- 55 terms correspond to these parent GO terms for mitochondrial function
...
nrow(rlog_BD_10)
```
- rlog_BD_10 has all the genes that are sig DE with alpha=0.1 and does not contain the p-val nor the adj p-val (562 genes in rarefied data)

- Now search within this list for all genes that correspond to the GO terms mitochondrial membrane function and call this GO_mit_sig
```{r,eval=FALSE}
GO_mit_sig = cc %>%
  filter(str_detect(term, "GO:0044455|GO:0044429")) %>%
...
```
- We have 5 genes, plot them in a heatmap but this time use the calculated difference from the mean
```{r,eval=FALSE}
...
nrow(rlog_BD_01)
```
- alpha of 0.05 yields 279 terms
- alpha of 0.01 yields 68 terms
...
- Only 18 out of the 30 DEGs had annotated GO terms
- sigDEG_BD_01 are all my significant DEGs between B and D in the right format with pval of 0.01
- Only 41 out of the 68 DEGs had annotated GO terms
